    ...
    for r in sorted_routs:
        if r['path'] == 0 and max_messages_bin_gar != 0:  # Binance -> Grantex
            ...
            text = r['bank'] + ' -> ' + r['coin'] + ' -> ' + 'Garantex' \
                   ...
                   + 'Получаем на binance: ' + f'{round(100_000 / r["binance_p2p_price"], 3)}' + f' {r["coin"]}\n' + '\n' \
                   + f'‣Продаем на [Garantex]({r["garantex_href"]}) ' + f'{r["coin"]}' + ' *по курсу' + f' {str(round(r["garantex_price"], 5))}*' \
                   + f'\nполучаем {round(100_000 / r["binance_p2p_price"] * r["garantex_price"], 3)} рублей'
            ...
            text = 'Garantex RUB' + ' -> ' + r['coin'] + ' -> ' + r['bank'] \
                   ...
                   + 'Получаем на Garantex: ' + f'{round(100_000 / r["garantex_price"], 3)}' + f' {r["coin"]}\n' + '\n' \
                   + f'‣Продаем на [binance p2p]({r["binance_p2p_href"]}) ' + f'{r["coin"]}' + ' *по курсу' + f' {str(round(r["binance_p2p_price"], 5))}*' \
                   + f'\nполучаем {round(100_000 / r["garantex_price"] * r["binance_p2p_price"], 3)} рублей ' + r[
                       'bank']
            messages.append(text)
